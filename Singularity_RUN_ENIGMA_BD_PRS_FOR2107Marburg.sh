...
export Project_Path=/lustre/scratch/data/fdavid_hpc-ENIGMA_BD_PRS
...
## LOO BD GWAS sumstats needed for FOR2107 data
export Sumstats_Dir=${Project_Path}/sumstats_FOR2107
...
do
	...
	if [ -f "${ancFile}.fam" ];
		...
		then
		   ...
		   # do not run - EAS in FOR2107 Marburg is only n=2
			echo "${ancFile}.fam contains only $(wc -l ${ancFile}.fam) samples"
